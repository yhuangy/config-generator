...
st.set_page_config(
    page_title="Config.py Generator",
    layout="wide",
)
...
st.markdown(
    ...
    and columns whose types you want displayed.
    </div>
    """,
...
for i in range(num_dfs):
    with st.sidebar.expander(f"📑 Configure DataFrame {i+1}", expanded=True):
        name = st.text_input(f"Name of DataFrame {i+1}", value=f"df1" if i == 0 else f"df{i+1}", key=f"name{i}")
...
if st.button("✅ Generate config.py"):
    st.success("config.py generated")
